...
4. **Blind/Unsupervised Feature Discovery:** We can go a step further and abandon labels entirely. Repeated patterns across a training set form distinct, non-Gaussian clusters in high-dimensional space, we can search for particular distribution bands (using Independent Component Analysis, Excess Kurtosis, Bimodality Coefficients, and KL-Divergence). This allows us to mathematically sift the latent space and discover features the network learned *completely unsupervised*.
